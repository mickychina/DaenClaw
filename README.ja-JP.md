...
セルフホストのアップデートサーバーを利用する場合は、**設定 → アップデート** を開き、
**アップデートサーバー URL** にベース URL（例: `https://updates.example.com`）を設定します。
ClawX は自動的にチャネルパス（`/latest`、`/beta`、`/dev`）を追加します。
